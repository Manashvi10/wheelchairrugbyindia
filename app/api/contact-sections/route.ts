...
export async function GET() {
  try {
    await ensureTable();
    const [rows] = await pool.execute("SELECT * FROM contact_sections");
    ...
    console.error("Contact sections GET error:", error);
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    await pool.execute(
      `INSERT INTO contact_sections (section_key, data, is_enabled)
       VALUES (?, ?, ?)
       ON DUPLICATE KEY UPDATE data = VALUES(data), is_enabled = VALUES(is_enabled)`,
      [section_key, JSON.stringify(data), is_enabled ? 1 : 0]
    );

    await logActivity("edit", `Updated contact section: ${section_key}`, auth.name);

    return NextResponse.json({ success: true });
  } catch (error) {
    console.error("Contact sections PUT error:", error);
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}
